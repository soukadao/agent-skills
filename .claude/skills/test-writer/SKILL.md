...
- [ ] テスト名が明確で、何をテストしているか理解できる
- [ ] AAA パターンに従っている
- [ ] 正常系と異常系の両方をカバーしている
- [ ] テストが独立して実行可能
- [ ] モックが適切に使用されている
- [ ] テストが失敗したとき、原因がわかりやすい
- [ ] 不要な複雑さがない（削除可能性）

## 使用例

テストコードを書く際は、以下のように依頼してください：

```
user.ts のテストコードを書いてください
```

または

```
このバリデーション関数のエッジケースをテストしてください
```

Claude は自動的にこのスキルを使用して、上記の原則に基づいたテストコードを生成します。
